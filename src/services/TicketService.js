...
class TicketService {
    ...
    async getTicketById(ticketId) {
        ...
        query = {
            text: 'SELECT users.user_name as "resolutionName", resolution.resolution_content as "resolutionContent", resolution.resolution_resolve_at as "resolutionTime", users.user_role as "resolutionUserRole" FROM resolution JOIN users ON users.user_id = resolution.resolution_resolve_by WHERE resolution.resolution_ticket = $1',
            values: [ticketId]
        }
        result = await this._pool.query(query)
